...
LineProjectionFactor::LineProjectionFactor(const Eigen::Vector3d &_pts_start, const Eigen::Vector3d &_pts_end,
                                           ...
                                           const Eigen::Matrix3d _delta_R, const Eigen::Vector3d _delta_T)
...
bool LineProjectionFactor::Evaluate(const double *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Matrix<double, 3, 3> R_W_i = delta_R * R_w;
    Eigen::Matrix<double, 3, 1> T_w_i = delta_R * T_w + delta_T;
    Eigen::Matrix<double, 3, 3> R = b_c_R.transpose() * R_W_i.transpose();   // trans to camera frame
    Eigen::Matrix<double, 3, 1> t = -R * T_w_i - (b_c_R.transpose() * b_c_T);
    ...
    residual[0] = ((mea_u_start - u_start) * (mea_u_start - u_start) + (mea_v_start - v_start) * (mea_v_start - v_start));
    residual[1] = ((mea_u_end - u_end) * (mea_u_end - u_end) + (mea_v_end - v_end) * (mea_v_end - v_end));
